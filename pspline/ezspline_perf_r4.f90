...
program drive2
  ...
  real(r4) t(12)
  type(ezspline1_r4) :: spl1
  type(ezspline2_r4) :: spl2
  type(ezspline3_r4) :: spl3
  integer n, ier, i
...
subroutine setup1d(spl, ki, t)
  ...
  type(ezspline1_r4), intent(inout) :: spl
...
subroutine interp1d_point(spl, ki, t)
  ...
  type(ezspline1_r4), intent(in) :: spl
  integer, intent(in) :: ki
  real(r4), intent(out) :: t
...
subroutine interp1d_cloud(spl, ki, t)
  ...
  real(r4), dimension(:), allocatable :: x1, f
  ...
  icount = 1
        do i=1, n1
           x1(icount) = spl%x1min + &
                & (spl%x1max-spl%x1min)*real(i-1,r4)/real(n1-1,r4)
           icount = icount + 1
        enddo
...
subroutine interp1d_array(spl, ki, t)
  ...
  real(r4), dimension(:), allocatable :: x1
...
subroutine setup2d(spl, ki, t)
  ...
  type(ezspline2_r4), intent(inout) :: spl
  integer, intent(in) :: ki

  real(r4) :: x1(ki), x2(ki), f(ki,ki)
...
subroutine interp2d_point(spl, ki, t)
  ...
  type(ezspline2_r4), intent(in) :: spl
  integer, intent(in) :: ki
  real(r4), intent(out) :: t
...
subroutine interp2d_cloud(spl, ki, t)
  ...
  real(r4), dimension(:), allocatable :: x1, x2, f
     ...
     do j=1, n2
        do i=1, n1
           x1(icount) = spl%x1min + &
                & (spl%x1max-spl%x1min)*real(i-1,r4)/real(n1-1,r4)
           x2(icount) = spl%x2min + &
                & (spl%x2max-spl%x2min)*real(i-1,r4)/real(n2-1,r4)
           icount = icount + 1
        enddo
     enddo
...
subroutine interp2d_array(spl, ki, t)
  ...
  real(r4), dimension(:), allocatable :: x1, x2
  real(r4), dimension(:,:), allocatable ::  f
...
subroutine setup3d(spl, ki, t)
  ...
  type(ezspline3_r4), intent(inout) :: spl
  integer, intent(in) :: ki

  real(r4) :: x1(ki), x2(ki), x3(ki), f(ki,ki,ki)
  integer :: bcs1(2), bcs2(2), bcs3(2)
  real(r4) :: x_pt, y_pt, z_pt, t
  integer tics_per_second, tic, toc
...
subroutine interp3d_point(spl, ki, t)
  ...
  type(ezspline3_r4), intent(in) :: spl
  integer, intent(in) :: ki
  real(r4), intent(out) :: t
...
subroutine interp3d_cloud(spl, ki, t)
  ...
  real(r4), dimension(:), allocatable :: x1, x2, x3, f
     ...
     do j=1, n2
        do i=1, n1
           ...
           x2(icount) = spl%x2min + &
                & (spl%x2max-spl%x2min)*real(i-1,r4)/real(n2-1,r4)
           x3(icount) = spl%x3min + &
                & (spl%x3max-spl%x3min)*real(i-1,r4)/real(n3-1,r4)
           icount = icount + 1
        enddo
     enddo
  enddo
...
subroutine interp3d_array(spl, ki, t)
  ...
  real(r4), dimension(:), allocatable :: x1, x2, x3
  real(r4), dimension(:,:,:), allocatable ::  f
