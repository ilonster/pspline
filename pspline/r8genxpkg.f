...
c
c     xpkg(j,1) = x(j), j = 1 to nx    ... nx.ge.3
c
...
c     xpkg(nx,3) = 1/<h>
c
c     xpkg(1,4) = 0.0 if x is *evenly spaced* (to ztol or 5.e-7 rel) else:
...
c     xpkg(3,4) = +/- tolerance epsilon for out-of-range warnings/errors
c                 +if message is to be written on out of range errors
c                 -if no message to be written.  In either case, a
c                        warning flag is set.
c
c     (if nx=3, xpkg(4,4) is not set; treated as 0.0 in lookup routine)
c
...
c
      if(nx.lt.3) then
         write(6,*) ' %genxpkg:  nx.ge.3 required!'
         ier=1
         return
      else
         ier=0
...
c
               if(abs(zh-xpkg(nx,2)).gt.ztola) xpkg(1,4)=1.0_r8
c
               xpkg(ix,2)=zh
               xpkg(ix,3)=1.0_r8/zh
c
